...
Static, hand-authored Pine Script v5 fragments for the "Smaug Entry Model" TradingView indicator. The daily routine that synthesizes `entry_models` rows fetches these files **verbatim** from GitHub and concatenates them with one freshly-generated block (the day's entry/exit rule thresholds) to produce the final `pinescript` column. This exists so the deterministic parts of the indicator (candlestick pattern math, RSI, the entry/exit marker convention) never drift day to day — only the rule thresholds, which are supposed to change as new data comes in, are LLM-generated.
