...
Whale Action and Roaring Kitty used to be tasks 5 and 6 here. They are
whole-market single-name screens, not SPY work, and they now run in their own
routine against the `erebor_candidates` table —
`docs/erebor-routine-prompt.md`. Do not write `daily_briefs.whales` or
`daily_briefs.squeezes`; those columns are retired and nothing reads them.
...
2. Query `analysis_runs` — `order by generated_at desc limit 1` — for the current regression/correlation output (correlations, standardized coefficients, r2_train/r2_test, deciles, and the `patterns` hit-rate block, per target).
...
The binary `pat_*` candlestick features are available as rule conditions (`{"feature": "pat_bull_engulf", "op": ">=", "value": 1}`). Read them out of the `patterns` block, not the correlations — a pattern that fires on a fraction of a percent of bars has a correlation of roughly zero no matter how well it works, so judge it on `edge_bps`, or `outcome.win_rate` against `baseline.win_rate`, and ignore any pattern the block omits (it fired too rarely to say anything). They're sparse by nature, so a pattern condition alone can drop a list's firing rate below the useful range: use one as the *discriminating* condition with one or two loose confirmations around it, not as a fifth `and` on an already-tight list.
...
Name all four of `daily_briefs`, `entry_models`, `trade_feedback`, and `training_examples.analysis_notes` explicitly, each with what you wrote or why you didn't. A part that was silently skipped looks identical to a part that succeeded unless you list them one by one — this is how `trade_feedback` stayed empty for days without anyone noticing, and how `whales` and `squeezes` went out empty on 2026-09-08 while the rest of the brief looked healthy. Do not report a write you did not verify.
