...
- **Swing pivots are the one feature with a non-obvious causality trap.** A bar isn't knowable as a local high until `SWING_RIGHT` later bars fail to exceed it, so `compute_features()` delays confirmed pivots by exactly that many bars. Drop the shift and every structure feature silently sees the future. `SWING_LEFT`/`SWING_RIGHT` (both 5) are the tunables. `bos`/`choch` are latched per pivot level — un-latched, they flicker every time price oscillates around a swing.
...
- **Fetch order is fixed:** `header.pine` → `rsi_9_21.pine` → `candles_1/2/3.pine` → `structure.pine` → *generated rules block* → `markers.pine`. `header.pine` must be first (`//@version=5` can have nothing before it); `markers.pine` must be last (it consumes `longEntry`/`shortEntry`/`exitSignal`, defined only by the generated block) and edge-triggers them, so the generated block defines plain state with no once-only guard of its own.
